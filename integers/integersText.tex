...
2.	Chips: A black chip represents one positive unit (e.g., 1 dollar in profit) and a red chip represents one negative unit (e.g., 1 dollar in debt).  If I have one black cchip together with one red chip, I have the same amount in value as if I have no chips at all (0).  That is, in the chip representation, one black chip “cancels” one red chip.
3.	Number Line:  Now one can go to the left of the zero you set.
Comparison of Integers
Like with the other number systems we’ve studied, we would like to discuss having two integer amounts and be able to tell which one is greater, lesser, or if the two amounts are equal.
While we could just “state a rule” for this, as with all mathematics, if it is possible to come up with a rule from a sense-making point of view, it will be more understandable and useful.
Note that, because, with allowing negative numbers, we are often not dealing with sets of objects that one can see and thus a physical one-to-one correspondence (even with the different-colored chips) is difficult to attain here.
However, like in other number systems, we can look at “real-life” situations and ask, say, which case someone has a better financial worth than another, better or worse rushing statistics in football, hotter (more) or colder (less) temperature, etc.  In each case, one needs to define which attribute means greater and which means lesser (e.g., in golf, the more negative score is a “better” score).
We can also use a number line and take advantage of what we did with positive numbers when saying that the number on the right is the greater number.  For example, -5 < 2 because -5 is to the left of 2 on the number line (Note: In some situations, one might say -5 is “greater” because it is further from zero, but those are in situations where we are not comparing integers, but only the absolute values of these integers to measure, for example, distance).
