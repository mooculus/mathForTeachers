...
Lines are \wordChoice{\choice[correct]{one-dimensional} \choice{two dimensional} \choice{three dimensional}}, meaning they have no thickness. However, in this case, our little bug could move in the forward-back direction, just not right-left.
...
\begin{image}
	...
	\end{tikzpicture}
\end{image}
Imagine that the edges drawn aren't actually edges, but keep extending forever. Even though this plane is drawn in three-dimensional space, the plane itself is \wordChoice{\choice{one-dimensional} \choice[correct]{two dimensional} \choice{three dimensional}}, because if our friend the little bug lived on this plane, it could move forward-back and also right-left as well as all of the combinations of those two directions. But since the plane has no thickness, the little bug couldn't fly around in an up-down direction.
