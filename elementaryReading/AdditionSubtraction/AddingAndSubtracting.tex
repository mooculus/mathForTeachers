...
\section{Activities for this section:} Adding it all up, New activity?, Mental Math with Addition
...
\begin{definition}
When we \dfn{add} two numbers $A$ and $B$ together, we combine the two quantities $A$ and $B$ and count the total. We write this total as $A+B$.
\end{definition}
...
\begin{definition}
When we \dfn{subtract} two numbers, we take the quantity $B$ away from the quantity $A$ and then count the remaining objects. We write this total as $A - B$.
\end{definition}
...
\emph{Carlos has $4$ pencils in his pencil box, and then he gives away $3$ pencils to his friend Darius. How many pencils does Carlos have now?}
