...
To record what we've done in our algorithm, notice that we crossed off one superbundle and we have two superbundles left. So, let's cross off the $3$ in $305$ and replace it with a $2$. (Remember this is the borrowing step.) We now have $10$ bundles, so let's cross off the $0$ in $305$ and replace it with a $10$.
...
Again, we have done this with a specific example, but we hope you will practice with more examples and see why the algorithm works for any two numbers, whether they are whole numbers or decimals. Pay special attention to the carrying step, which can be a little confusing. But when we did the carrying step, each time we were bundling up one kind of object into the next kind of object, and so we had to place them in the next place value to the left.
