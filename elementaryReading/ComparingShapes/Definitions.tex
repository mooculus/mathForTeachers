...
\begin{definition}
	An \dfn{edge} of a shape is a straight line segment that we use to draw a shape. For example, the shape below has $5$ edges. We will sometimes use the word ``sides'' instead of edges, but in general edges have to be straight and sides could be curved.  
	\begin{image}
		\begin{tikzpicture}
...
\begin{definition}
	A polygon is called \dfn{regular} if all of its edges are the same length. Here is an example of a regular polygon.
	\begin{image}
\begin{tikzpicture}
% Number of sides
\def\n{5}
