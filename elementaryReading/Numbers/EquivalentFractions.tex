...
Notice that we cannot only cut our equal pieces into more pieces, but we can fuse them back together into bigger equal pieces, as long as what we end up with are actually equal pieces! For instance, start with the fraction $\frac{4}{6}$. Here is the same picture we drew earlier.
...
\]
Notice that this second idea involves multiplying two fractions together, which we also haven't talked about yet. For now, if you need to write down the rule in general, we will prefer the first version over the second. And we will return to all of these ideas after we've talked about multiplication so that we can explain why all of this makes sense and fits with our initial thinking about equivalent fractions. For now, practice explaining your thinking based on the meaning of equivalent fractions and counting the pieces like we did in the previous example.
