...
In a previous activity, you should have come to the conclusion that if lengths are scaled by $k$, then areas are scaled by $k^2$ (the ``area scale factor'') and volumes are scaled by $k^3$ (the ``volume scale factor'').  We apply this reasoning to the problems in this activity. You might review the instructor notes in ``Scale it up'' for some hints on how to help students with the notion of scaling an individual unit of area or volume.
