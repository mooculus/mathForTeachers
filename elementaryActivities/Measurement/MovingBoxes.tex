...
The third problem uses a standard unit of measure, and so students should be allowed to use the formula for volumes of boxes if needed. You could point out why this was not allowed on the previous problem. One nice strategy here might be to place a cube on each area unit, then just cut the prism into five different layers, looking ahead to the $V = A \times h$ formula that we will justify later.
