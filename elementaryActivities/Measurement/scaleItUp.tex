...
\item For the ``individual unit of area'', we are actually dealing with two different units here: the original and the scaled unit. This can be a sticking point for groups, and when groups get stuck here they can get very stuck.

\item When discussing how many of the original units fit inside the scaled unit, we are essentially finding a conversion factor (but we don't need to discuss this here). If k is the scaling factor, the scaled unit fits ($k^2$ for area, $k^3$ for volume) original units. Encourage students to draw a picture to justify this fact, connecting back to the geometric argument where we fit original units into the scaled unit.

\item When we scale from the original to the new, the number of units does not change. If we started with $16$ square units, we now have $16$ of the scaled square units. This can be a tricky one for students, especially if you try drawing a picture to show what's happening and fitting the smaller units into the larger ones. We are really thinking about how each of the scaled units is related to the original units. For example, if we started with $16$ square units and scale by a factor of $k=3$, we have $9$ original units inside each of the $16$ scaled units. $16$ groups, with $9$ original units per group gives us $16 \times 9 = 144$ original units in the scaled version.

\item All together, scaling the unit requires understanding the previous 3 points. For several students there was an ``ah ha" moment where they figured out their individual sticking point, and after that they could quickly work out how to bring it all together. Don't hesitate to go slowly and carefully!


\item 
As an extension, this idea is related to why a 12-inch pizza doesn't cost twice as much as a 6-inch pizza. The area scale factor isn't the same as the length scale factor! This is a nice extension for students who get to the point quickly.
\end{itemize}
...
{\bf Good language:} Many times, if students understand what is happening with 2D scaling, they can extend to 3D scaling. So if students are having trouble with 3D scaling, you might go back to the 2D case and make sure everything makes sense there.


Students may start seeing some comparisons here to our conversions with area and volume. You can encourage this discussion if students find it helpful. 
