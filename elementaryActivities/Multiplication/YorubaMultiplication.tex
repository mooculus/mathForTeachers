...
\begin{problem}
Marcus thinks that $17 \times 19$ is the same as $16 \times 20$ because we decreased $17$ by one and increased $19$ by one and that should balance out. How could you use the pictures above to help Marcus understand why this thinking is incorrect?
\end{problem}
...
\begin{itemize}
	...
	\item In Problem 2 the idea is that if we take one of the groups and make it into objects, perhaps we haven't changed the total? But since the size of a group is different from the number of objects in one group, the total won't match up exactly. You can also try drawing a single array with 17 rows and 19 squares per row and see if they can be ``rearranged'' into 16 rows with 20 squares per row. Yet another way to think about what's happening here is to overlay two pictures (one for each multiplication) and see that the ``extras'' don't match up.
